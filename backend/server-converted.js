...
app.get('/api/training/trainer-topics/:trainerName', (req, res) => {
  ...
  dbAll(sql, [trainerName]).then(rows => res.json({
      trainer: trainerName,
      topics: rows
...
app.post('/api/training', (req, res) => {
  ...
  dbRun(sql, [String(employee_id), role || 'trainee', training_date, training_topic, training_hours, trainer_name, notes, session_id || null]).then(result => { /* converted */ }).catch(err => res.status(500).json({ error: err.message }));
    }
    res.json({
      id: this.lastID,
...
app.get('/api/training/employee/:id', (req, res) => {
  ...
  dbAll(sql, [id]).then(rows => res.json(rows)).catch(err => res.status(500).json({ error: err.message }));
});
...
app.get('/api/training/search', (req, res) => {
  ...
  dbAll(sql, [`%${query}%`, `%${query}%`]).then(rows => res.json({ results: rows })).catch(err => res.status(500).json({ error: err.message }));
});
...
app.get('/api/training/leaderboard/trainees', (req, res) => {
  ...
  dbAll(sql, [limit]).then(rows => res.json({ leaderboard: rows })).catch(err => res.status(500).json({ error: err.message }));
});
...
app.delete('/api/admin/reset-employee-training/:employeeId', (req, res) => {
  ...
  dbRun('DELETE FROM training_records WHERE employee_id = ?', [employeeId]).then(result => { /* converted */ }).catch(err => res.status(500).json({ error: err.message }));
    }
